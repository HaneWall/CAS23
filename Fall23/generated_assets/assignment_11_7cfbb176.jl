...
#> description = "Abgabe 23.01.2025, 23:59 Uhr"
...
md"""
**Abgabe bis**: 23.01.2025 - 23:59 Uhr auf StudIP im "Aufgaben"-Menü der Vorlesung.
"""
...
md"""
## Teil 1: Fortsetzung - WWW und Page-Rank-Algorithmus von Google
"""

# ╔═╡ 89f82cff-f9b0-4308-8a41-cc6bb4f7c827
md"""
> #### Preliminaries
>> Auf den letzten Aufgabenblatt haben wir mithilfe der `eigen(A)` Funktion alle Eigenwerte und die jeweiligen Eigenvektoren einer Bewertungsmatrix $A$ ermittelt. Fuer den Page-Rank Algorithmus benoetigen wir jedoch nur den Eigenvektor zum Eigenwert 1. Im fortlaufenden wiederholen wir die Aufgabe, nutzen jedoch die sogenannte Potenzmethode.
"""
...
md"""
> #### Aufgabe 1a) 
>> Die Bewertungsmatrix $A$ des in der Vorlesung behandelten Page-Rank-Algorithmus ist eine sogenannte spalten stochastische Matrix, bei der die Summe der Elemente jeder Spalte jeweils 1 ergibt. Diese stochastischen Matrizen haben die Eigenschaft, dass die iterative Berechnung von

>> $x^{(k+1)} = Ax^{(k)} \quad \text{mit gegebenen Startvektor} \qquad x^{(0)} \neq 0$

>> für große $k$ gegen den zum Eigenwert 1 (welches der betragsgrößte Eigenwert ist) zugehörigen Eigenvektor, d.h. gegen die Lösung der Gleichung $x = Ax$ konvergiert. Dies soll im Folgenden getestet werden. Betrachten Sie das in Aufgabe 1 dargestellte Beispiel eines kleinen Netzes mit 8 Webseiten und verwenden Sie die dort berechnete Link-Bewertungsmatrix $A = A_{ij} \in \mathbb{R}^{8\times8}$ für die beschriebene Iteration. Verwenden Sie den Startvektor 

>> $x^{(0)} = (1/8, ..., 1/8)^{T}$ 

>>und schreiben Sie eine Julia-Funktion, welche basierend auf der Konnektivitätsmatrix den Relevanzvektor $x$ des in der Vorlesung behandelten Page-Rank-Algorithmus mit $k_{max} = 50$ Iterationen berechnet. Normieren Sie zusätzlich nach jedem Iterationsschritt den berechneten Vektor $x^{(k)}$ und vergleichen Sie die Ergebnisse.
"""
...
md"""
!!! warning "Aufgabe 2b)"
	Berechnen Sie die Relevanzliste iterativ.  Hierfür sei die geforderte Genauigkeit der Funktion $\varepsilon = 0.001$. Geben Sie die Anzahl der notwendigen Schritte aus, bis diese Genauigkeit erreicht ist.
"""

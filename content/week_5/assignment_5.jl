...
md"""
## Aufgabe 1: Die logistische Abbildung
"""

# ╔═╡ e60bcfb7-9ccc-461b-bf9c-502adbd0fe83
md"""

Es soll die Abbildung $f:\mathbb{R}\rightarrow \mathbb{R}$ mit $f(x)=\lambda x(1-x)$
und $\lambda\in(1, 4)$
iterativ angewendet werden.
"""
...
md"""
!!! danger "1e*)"
	Welche Schlussfolgerung kann aus b)-d) erhalten werden? Wie
...
md"""
Es soll eine Nullstelle der Funktion $f:\mathbb{R}\rightarrow \mathbb{R}$, $x\mapsto
 f(x)$ und 

$f(x)=-x^3-2x^2+x+1$

numerisch im Intervall [-2,0] ermittelt werden. Berechnen Sie die ersten 8 Iterierten des Newton-Verfahrens mit dem Startwert $x_0=0$. Implementieren Sie dazu das Newton-Verfahren. 

Das Programm soll dabei den Startwert $x_0$, eine Funktion (z.B. $f(x)$) und die Anzahl der durchzuführenden Iterationsschritte einlesen. Eine Bestimmung von Ableitungen soll mit Hilfe von dem Paket "ForwardDiff" erfolgen oder an das Computerprogramm übergeben werden.
"""

# ╔═╡ 196662e9-83df-436b-a37d-e06ccc7be4e7
md"""
## (Zusatz)aufgabe 3 - Steffensen Verfahren
"""
...
md"""
!!! danger "3b*)"
	Bestimmen Sie einmal mittels des Newton- und einmal mittels des Steffensen-Verfahren die Approximation der Nullstelle der Funktion 
    
    $f(x)=x^2-1$
	
	mit Startwert $x_0 = 1.5$ nach $10$ Iterationsschritten.
    Speichern Sie die Ergebnisse nach jedem Iterationsschritt mit Hilfe einer Zeilenmatrix und stellen Sie diese anschließend graphisch dar. Beachten Sie hierbei, auch den Startwert abzubilden.
"""
...
md"""
!!! danger "3c*)"
	Führen Sie Aufgabe b) auch für die Funktion   
    $f(x)= -38x^3+1$
    durch, mit Startwert $x_0 = 2$ nach $5$ Iterationsschritten. Vergleichen Sie Ihr Ergebnis mit dem aus Aufgabenteil b). Was stellen Sie fest?
"""
...
md"""
Ableitungsberechnung f\"ur eine Funktion mit einer Variablen. Gegeben sei die Funktion 

$f(x)=-\frac{1}{2} x^3 + x^2 -1$.
"""
...
md"""
!!! warning "4c)"
	Berechnen Sie mit der Definition der Ableitung als Grenzwert
	eines Differenzenquotienten die erste Ableitung von $f(x)$ an der
	Stelle $x_0=1$. Formulieren Sie hierzu den Differenzenquotient und
	berechnen Sie anschließend den Grenzwert analytisch.
"""
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "669aab34bcb65646c45e3786a88d5ae1771ccfe6"
